import React from 'react';

const FeaturesOne = () => {
        return (
            <div className="cta-three">

                <img src={require("../../../assets/images/shapes/cta-1-shape-2.png")} alt="" className="cta-three__shape-2" />
                    <img src={require("../../../assets/images/shapes/cta-1-shape-3.png")} alt="" className="cta-three__shape-3" />
                        <div className="container">
                            <img src={require("../../../assets/images/shapes/cta-1-shape-1.png")} alt="" className="cta-three__shape-1" />
                                <img src={require("../../../assets/images/mocs/cta-moc-1-1.png")} alt="" className="cta-three__moc" />
                                    <div className="row justify-content-end">
                                        <div className="col-lg-6">
                                            <div className="cta-three__content">
                                                <div className="block-title text-left">
                                                    <span className="block-title__bubbles"></span>
                                                    <p>Checkout Our Services</p>
                                                    <h3>Control Everything From One Application</h3>
                                                </div>
                                                <div className="cta-three__box-wrap">
                                                    <div className="cta-three__box">
                                                        <div className="cta-three__box-icon">
                                                            <i className="zimed-icon-strategy"></i>
                                                        </div>
                                                        <div className="cta-three__box-content">
                                                            <h3>Feature Here</h3>
                                                            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit,
                                                                sed do eiusmod tempor anagi
                                                                icdunt ut labore et dolore magna aliqua.</p>
                                                        </div>

                                                    </div>

                                                    <div className="cta-three__box">
                                                        <div className="cta-three__box-icon">
                                                            <i className="zimed-icon-training"></i>
                                                        </div>

                                                        <div className="cta-three__box-content">
                                                            <h3>Feature Here</h3>
                                                            <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit,
                                                                sed do eiusmod tempor anagi
                                                                icdunt ut labore et dolore magna aliqua.</p>
                                                        </div>

                                                    </div>
                                                </div>

                                                <a href="#" className="thm-btn cta-three__btn">Discover More</a>
                                            </div>

                                        </div>

                                    </div>
                        </div>
            </div>

        )
}
export default FeaturesOne;
